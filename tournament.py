...
from sample_players import PredictablePlayer
...
from game_agent import player_weights
from game_agent import in_isolation
import sys

NUM_MATCHES = 5  # number of matches against each opponent #was 5
TIME_LIMIT = 150 # number of milliseconds before timeout #was 150
...
def play_round(agents, num_matches):
    ...
    for idx, agent_2 in enumerate(agents[:-1]):
        ...
        start_killer_moves = len(transposition_table.keys())
        ...
        end_killer_moves = len(transposition_table.keys())

        print("Keys added per second: {}, increment: {}, time taken: {}".format((end_killer_moves - start_killer_moves) / (end - start), end_killer_moves - start_killer_moves, end - start))
...
def main():
    if len(sys.argv) >= 2:
        player_weights["player"] = float(sys.argv[1])
    ...
    AB_ARGS = {"search_depth": 5, "method": 'alphabeta', "iterative": False} # was 5
    MM_ARGS = {"search_depth": 3, "method": 'minimax', "iterative": False}   # was 3
    ...
    # removing random agents since they do not contribute to learning anymore
    #random_agents = [Agent(RandomPlayer(), "Random")]
    random_agents = [Agent(RandomPlayer(), "Random"), Agent(GreedyPlayer(), "Greedy"), Agent(PredictablePlayer(), "Predictable")]
